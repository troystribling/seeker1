...
//-----------------------------------------------------------------------------------------------------------------------------------
static SeekerDbi* thisSeekerDbi = nil;

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
@interface SeekerDbi (PrivateAPI)

@end

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
@implementation SeekerDbi
...
//===================================================================================================================================
#pragma mark SeekerDbi

//-----------------------------------------------------------------------------------------------------------------------------------
+ (SeekerDbi*)instance {	
    @synchronized(self) {
        if (thisSeekerDbi == nil) {
            thisSeekerDbi = [[self alloc] init]; 
        }
    }
    return thisSeekerDbi;
}
...
- (BOOL)copyDbFile {	
	...
	dbFilePath = [documentFolderPath stringByAppendingPathComponent:@"seeker.db"];
	[dbFilePath retain];	
	if (![[NSFileManager defaultManager] fileExistsAtPath: dbFilePath]) {
		NSString *backupDbPath = [[NSBundle mainBundle] pathForResource:@"seeker" ofType:@"db"];
		if (backupDbPath == nil) {
			return NO;
		} else {
...
- (BOOL)open {
	if (sqlite3_open([dbFilePath UTF8String], &sqlDb)) { 
		NSLog (@"error opening database: seeker.db"); 
		return NO;
	} 
	return YES;
}
...
//===================================================================================================================================
#pragma mark SeekerDbi PrivateAPI
...
+ (id)allocWithZone:(NSZone *)zone {	
    @synchronized(self) {
        if (thisSeekerDbi == nil) {
            thisSeekerDbi = [super allocWithZone:zone];
            return thisSeekerDbi;
        }
    }
    return nil;
